...
public class ManifestProviderTests
{
    ...
    public async Task GetManifestAsync_WithGameInstallation_BuildsCorrectManifestId()
    {
        ...
        var expectedManifest = new ContentManifest
        {
            Id = "1.0.eaapp.generals-installation",
            Name = "Test Manifest",
        };
        ...
        // Assert
        Assert.NotNull(result);
        Assert.Equal("1.0.eaapp.generals-installation", result.Id);
        _poolMock.Verify(x => x.GetManifestAsync(ManifestId.Create("1.0.eaapp.generals-installation"), default), Times.Once);
    }
    ...
    public async Task GetManifestAsync_WithZeroHourInstallation_UsesZeroHourId()
    {
        ...
        var installation = new GameInstallation(
            ...
            logger: null);
        installation.SetPaths(null, @"C:\TestPath\Command and Conquer Generals Zero Hour"); // Only ZeroHour path to force zerohour ID
        var expectedManifest = new ContentManifest
        {
            Id = "1.0.steam.zerohour-installation",
            Name = "Test Manifest",
        };

        _poolMock.Setup(x => x.GetManifestAsync(It.Is<ManifestId>(id => id.Value == "1.0.steam.zerohour-installation"), It.IsAny<CancellationToken>()))
                  .ReturnsAsync(OperationResult<ContentManifest?>.CreateSuccess(expectedManifest));
        ...
        // Assert
        Assert.NotNull(result);
        Assert.Equal("1.0.steam.zerohour-installation", result.Id);
        _poolMock.Verify(x => x.GetManifestAsync(It.Is<ManifestId>(id => id.Value == "1.0.steam.zerohour-installation"), It.IsAny<CancellationToken>()), Times.Once);
    }
