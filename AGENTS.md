...
- When babysitting: poll checks and all bot comments (including inline review threads and summary 'Outside diff range' findings) newer than the last push. Verify each finding against the source, fix real ones in code, reply to review comments where appropriate, and resolve each discussion thread on GitHub using the GraphQL mutation (`resolveReviewThread`). Never leave addressed review threads unresolved. For extended PR workflows, invoke the `pull-request` and `babysit-pr` skills. Stay quiet when nothing is new. Stop when all checks pass on the latest commit with all threads resolved.
