...
- When babysitting: poll checks, check-run annotations (including SonarCloud Quality Gate issues via `gh api repos/:owner/:repo/check-runs/:id/annotations`), and all bot comments (including inline review threads and summary 'Outside diff range' findings) newer than the last push. Verify each finding against the source, fix real ones in code (including all SonarCloud/DeepSource/static analysis warnings), reply to review comments where appropriate, and resolve each discussion thread on GitHub using the GraphQL mutation (`resolveReviewThread`). Never leave addressed review threads unresolved or static analyzer issues ignored. For extended PR workflows, invoke the `pull-request` and `babysit-pr` skills. Stay quiet when nothing is new. Stop when all checks pass on the latest commit with all threads resolved and zero static analysis issues.
