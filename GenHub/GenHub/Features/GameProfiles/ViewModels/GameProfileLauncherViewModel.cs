    ...
    ILogger<GameProfileLauncherViewModel>? logger) : ViewModelBase
{
                ...
                if (_profileEditorFacade != null && _gameProfileManager != null)
                {
                    foreach (var installation in installations.Data)
                    {
                        ...
                        if (installation.AvailableGameClients != null)
                        {
                            ...
                            {
                                var profileCreated = await TryCreateProfileForGameClient(installation, gameClient);
    ...
    private async Task<bool> TryCreateProfileForGameClient(GameInstallation installation, GameClient gameClient)
    {
        try
        {
            if (_profileEditorFacade == null || _gameProfileManager == null)
            ...
            if (existingProfiles.Success && existingProfiles.Data != null)
            {
                ...
                if (profileExists)
                {
                    _logger.LogDebug("Profile already exists for {InstallationType} {GameClientName} (matched by Name+InstallationId), skipping", installation.InstallationType, gameClient.Name);
                    return false;
                }
                ...
                {
                    _logger.LogDebug("Profile already exists for {InstallationType} {GameClientName} (matched by Name+ClientId), skipping", installation.InstallationType, gameClient.Name);
                    return false;
                }
            }

            // Get user's preferred workspace strategy or default
            var preferredStrategy = _configService?.GetDefaultWorkspaceStrategy() ?? WorkspaceStrategy.SymlinkOnly;

            var manifestVersion = gameClient.GameType == GameType.ZeroHour
            ...
            if (profileResult.Success && profileResult.Data != null)
            {
                _logger.LogInformation("Successfully created profile '{ProfileName}' for {InstallationType} {GameClientName}", profileResult.Data.Name, installation.InstallationType, gameClient.Name);
                return true;
            }
            else
            {
                var errors = string.Join(", ", profileResult.Errors);
                _logger.LogWarning("Failed to create profile for {InstallationType} {GameClientName}: {Errors}", installation.InstallationType, gameClient.Name, errors);
                return false;
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error creating profile for {InstallationType} {GameClientName}", installation.InstallationType, gameClient?.Name ?? "Unknown");
            return false;
        }
    }
