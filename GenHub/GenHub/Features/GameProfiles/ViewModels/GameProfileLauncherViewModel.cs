    ...
    private async Task<bool> TryCreateProfileForInstallation(GameInstallation installation, GameType gameType)
    {
        try
        {
            ...
            if (existingProfiles.Success && existingProfiles.Data != null)
            {
                ...
                if (profileExists)
                {
                    _logger.LogDebug("Profile already exists for {InstallationType} {GameType}, skipping", installation.InstallationType, gameType);
                    return false;
                }
            }
            ...
            var createRequest = new Core.Models.GameProfile.CreateProfileRequest
            {
                ...
                EnabledContentIds = new List<string> { gameClient.Id }, // Client manifest (installation implied by ID)
            };
    ...
    private async Task EditProfile(GameProfileItemViewModel profile)
    {
        ...
        try
        {
            ...
            if (mainWindow != null)
            {
                ...
                // Refresh the profiles list after the window closes
                await InitializeAsync();
