    ...
    ILogger<GameInstallationService>? logger = null) : IGameInstallationService, IDisposable
{
    ...
    {
        // Fetch installation details early to ensure Has* flags are set correctly
        foreach (var installation in installations)
        {
            installation.Fetch();
        }

        // Detect and populate clients (via orchestrator → detector) from pre-detected installations
        ...
        // Group game clients by InstallationId and populate each installation
        ...
        // Installations are already populated by detector (via PopulateGameClients)
        // Now generate installation manifest for each available game type in each installation
        foreach (var installation in installations)
        {
            if (_manifestGenerationService == null || _contentManifestPool == null) continue;

            var gameDir = installation.InstallationPath;
            if (!Directory.Exists(gameDir)) continue;

            // Generate for Generals if available
            if (installation.HasGenerals && !string.IsNullOrEmpty(installation.GeneralsPath) && Directory.Exists(installation.GeneralsPath))
            {
                ...
                var generalsIdString = $"{installation.InstallationType.ToString().ToLowerInvariant()}.generals-installation.1";
                generalsManifest.Id = ManifestId.Create(generalsIdString);
            ...
            // Generate for Zero Hour if available
            if (installation.HasZeroHour && !string.IsNullOrEmpty(installation.ZeroHourPath) && Directory.Exists(installation.ZeroHourPath))
            {
                ...
                var zeroHourIdString = $"{installation.InstallationType.ToString().ToLowerInvariant()}.zerohour-installation.1";
                zeroHourManifest.Id = ManifestId.Create(zeroHourIdString);
