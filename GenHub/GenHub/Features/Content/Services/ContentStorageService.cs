...
public class ContentStorageService : IContentStorageService
{
    ...
    private bool IsInvalidOrRemovableDrive(string path)
    {
        try
        {
            ...
            // Additional check: if it's A: or B: drive (common floppy drives)
            if (rootPath.Equals(@"A:\", StringComparison.OrdinalIgnoreCase) ||
                rootPath.Equals(@"B:\", StringComparison.OrdinalIgnoreCase))
            {
                _logger.LogWarning("Drive {Drive} is a legacy floppy drive, considering invalid", rootPath);
                return true;
            }
