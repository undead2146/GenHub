...
public class LocalFileSystemContentProvider : BaseContentProvider
{
    ...
    {
        _fileSystemDiscoverer = discoverers.FirstOrDefault(d => d.SourceName.Contains("Local File System"))
            ?? throw new InvalidOperationException("No FileSystem discoverer found");
        _localResolver = resolvers.FirstOrDefault(r => r.ResolverId.Contains("LocalManifest"))
            ?? throw new InvalidOperationException("No Local resolver found");
        _fileSystemDeliverer = deliverers.FirstOrDefault(d => d.SourceName.Contains("Local File System"))
