    ...
    IUserSettingsService userSettingsService) : IVelopackUpdateManager, IDisposable
{
        ...
        try
        {
            ...
            foreach (var run in runs.EnumerateArray())
            {
                ...
                foreach (var artifact in artifacts.EnumerateArray())
                {
                    ...
                    // Extract version from artifact name (format: genhub-velopack-windows-1.0.0-alpha.1.PR42.123)
    ...
    public async Task InstallPrArtifactAsync(PullRequestInfo prInfo, IProgress<UpdateProgress>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Prefer Windows nupkg (contains -win in filename), exclude Linux
            var nupkgFile = nupkgFiles
                .FirstOrDefault(f => Path.GetFileName(f).Contains("-win", StringComparison.OrdinalIgnoreCase))
            ...
            // Extract version from nupkg filename (e.g., GenHub-1.0.0-alpha.148-full.nupkg -> 1.0.0-alpha.148)
            ...
            // Point Velopack to localhost for download
            var source = new SimpleWebSource($"http://localhost:{port}/");
