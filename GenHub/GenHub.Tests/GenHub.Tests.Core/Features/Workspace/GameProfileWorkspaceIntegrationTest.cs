...
public class GameProfileWorkspaceIntegrationTest : IDisposable
{
    ...
    public async Task PrepareWorkspace_MixedContentTypes_HandlesGameInstallationAndGameClientCorrectly()
    {
        ...
        {
            Id = ManifestId.Create("test.game.installation.manifest"),
        ...
        {
            Id = ManifestId.Create("test.game.client.manifest"),
            ...
            Files = new List<ManifestFile>
            {
                ...
                new ManifestFile
                {
                    ...
                    SourceType = GenHub.Core.Models.Enums.ContentSourceType.GameInstallation,
                    Size = new FileInfo(Path.Combine(_tempGameInstall, "game.exe")).Length,
                    IsExecutable = true,
                },
            },
        };
        ...
        Assert.True(File.Exists(Path.Combine(workspacePath, "game.exe")), "GameClient executable should be present");

        // Verify executable path is set correctly
        // With the new implementation, GameClient manifest takes priority for executable resolution
        Assert.Equal(Path.Combine(workspacePath, "game.exe"), workspaceInfo.ExecutablePath);
    }
    ...
    public async Task PrepareWorkspace_LargeFiles_CopiesSuccessfully()
    {
        ...
        {
            Id = ManifestId.Create("test.large.file.manifest"),
            ContentType = GenHub.Core.Models.Enums.ContentType.GameInstallation,
            Files = new List<ManifestFile>
            {
                new ManifestFile
    ...
    public async Task PrepareWorkspace_OverlappingManifests_HandlesCorrectly()
    {
        ...
        {
            Id = ManifestId.Create("test.manifest.one.id"),
            ContentType = GenHub.Core.Models.Enums.ContentType.GameInstallation,
            Files = new List<ManifestFile>
            {
                new ManifestFile
        ...
        {
            Id = ManifestId.Create("test.manifest.two.id"),
    ...
    private void SetupMockServices()
    {
        ...
        var gameProfile = new GameProfile
        {
            ...
            EnabledContentIds = new List<string> { "test.game.installation.manifest", "test.game.client.manifest" },
    ...
    private List<ContentManifest> CreateTestManifests()
    {
        ...
        var testFiles = new[] { "generals.exe", "game.exe", "data/generals.big", "data/textures/texture1.tga", "mods/mod1/mod.ini" };
        foreach (var file in testFiles)
        {
            var fullPath = Path.Combine(_tempGameInstall, file);
