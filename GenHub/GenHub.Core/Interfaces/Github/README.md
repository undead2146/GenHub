...
- **IGitHubApiClient**: Interface for the low-level GitHub API client that handles HTTP communication.
    - **Key Methods**:
        - `GetStreamAsync(GitHubRepoSettings repo, string url)`: Fetches a resource as a stream (e.g., for downloading artifacts), potentially returning content length.
        - `GetArtifactsForRunAsync(GitHubRepoSettings repo, long runId)`: Specifically fetches artifacts for a workflow run.
...
- **IGitHubArtifactService**
    ...
    - **Key Methods**:
        - `GetArtifactsForWorkflowRunAsync(GitHubRepoSettings repo, long workflowRunId)`: Retrieves all artifacts associated with a specific workflow run.
        - `GetArtifactDetailsAsync(GitHubRepoSettings repo, long artifactId)`: Fetches detailed information for a single artifact.
...
- **IGitHubRepoService**
    ...
    - **Key Methods**:
        - `GetRepositoryDetailsAsync(GitHubRepoSettings repoSettings)`: Fetches general information about a repository.
        - `GetLatestReleaseAsync(GitHubRepoSettings repoSettings)`: Fetches the latest release for a repository.
        - `ListBranchesAsync(GitHubRepoSettings repoSettings)`: Lists branches in a repository.
...
- **IGitHubSearchService**
    ...
    - **Key Methods**:
        ...
        - `SearchCodeAsync(string query, GitHubRepoSettings repoContext)`: Searches for code within a specific repository or across GitHub.
...
- **IGitHubWorkflowRunService**
    ...
    - **Key Methods**:
        - `GetWorkflowRunsAsync(GitHubRepoSettings repo, string? workflowFileName = null, string? branch = null, string? eventType = null, int count = 30)`: Fetches a list of workflow runs for a repository, with optional filters.
        - `GetWorkflowRunDetailsAsync(GitHubRepoSettings repo, long runId)`: Fetches detailed information for a specific workflow run, including its jobs and steps.
        - `GetArtifactCountsForWorkflowsAsync(GitHubRepoSettings repo, IEnumerable<long> runIds)`: Efficiently gets artifact counts for multiple workflow runs.
