...
> [!IMPORTANT]
> **Architectural Decision: Definition vs. Catalog Format Boundary**
> - **Publisher Catalog (`catalog.json`)**: Contains publisher metadata and content items (`CatalogContentItem[]`). This is the **only format** that populates the UI.
> - **Provider Definition (`definition.json`, Publisher Studio)**: Contains publisher metadata and one or more catalog endpoints (`CatalogUrl`). Subscribing via definition resolves the definition to retrieve the direct `CatalogUrl`, then delegates to `GenericCatalogDiscoverer`.
> - **Guard Clause**: `GenericCatalogDiscoverer` throws `NotImplementedException` if `DefinitionUrl != null` until definition endpoint resolution is implemented.
...
- [x] Create shared design document in `/docs/dev/`

### Phase 1: Core Models & Interfaces (COMPLETED)

**Goal**: Define data contracts without implementation

| File | Description | Status |
|------|-------------|--------|
| `Core/Models/Providers/PublisherCatalog.cs` | Root catalog model | [x] Implemented |
| `Core/Models/Providers/CatalogContentItem.cs` | Content entry | [x] Implemented |
| `Core/Models/Providers/ContentRelease.cs` | Version-specific release | [x] Implemented |
| `Core/Models/Providers/ReleaseArtifact.cs` | Downloadable file | [x] Implemented |
| `Core/Models/Providers/PublisherSubscription.cs` | Local subscription record | [x] Implemented |
| `Core/Interfaces/Providers/IPublisherSubscriptionStore.cs` | Subscription CRUD | [x] Implemented |
| `Core/Interfaces/Providers/IPublisherCatalogParser.cs` | Catalog parsing | [x] Implemented |
| `Core/Interfaces/Providers/IVersionSelector.cs` | Version filtering | [x] Implemented |

### Phase 2: Subscription System (COMPLETED)

**Goal**: Enable adding/removing publisher subscriptions

- [x] Implement `PublisherSubscriptionStore` (file-based storage in `GenHub/Features/Content/Services/Catalog/PublisherSubscriptionStore.cs`)
- [x] Add subscribed publishers to `DownloadsBrowserViewModel.InitializePublishersAsync()` (`DownloadsBrowserViewModel.cs:655`)
- [x] Handle `genhub://subscribe?url=...` URI scheme via `SubscriptionConfirmationViewModel`

### Phase 3: Generic Pipeline (COMPLETED)

**Goal**: Discover and download content from subscribed catalogs

- [x] Implement `GenericCatalogDiscoverer` (`GenHub/Features/Content/Services/Catalog/GenericCatalogDiscoverer.cs`)
- [x] Implement `GenericCatalogResolver` (`GenHub/Features/Content/Services/Catalog/GenericCatalogResolver.cs`)
- [x] Integrate with existing `ContentManifestBuilder.AddDownloadedFileAsync()`
- [x] Add caching for fetched catalogs (`ContentCacheService.cs`)
